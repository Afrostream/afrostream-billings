			...
			while($dailyCounter < $lastdaysCount) {
				...
				if($s3->doesObjectExist($bucket, $dailyKey) == false) {
					...
					if(($export_subscriptions_file_path = tempnam('', 'tmp')) === false) {
						throw new Exception('file for exporting daily chartmogul afr subscriptions cannot be created');
					}
					$billingsExportAfrSubscriptions->doExportSubscriptionsForChartmogul($dayToProcessBeginningOfDay, $dayToProcessEndOfDay, $export_subscriptions_file_path);
					$s3->putObject(array(
							'Bucket' => $bucket,
							'Key' => $dailyKey,
			...
			if($dayOfMonth >= $firstDayToProceedLastMonth) {
				while($monthlyCounter < $lastmonthsCount) {
					...
					if($s3->doesObjectExist($bucket, $monthlyKey) == false) {
						$export_subscriptions_file_path = NULL;
						if(($export_subscriptions_file_path = tempnam('', 'tmp')) === false) {
							throw new Exception('file for exporting monthly chartmogul afr subscriptions cannot be created');
						}
						$billingsExportAfrSubscriptions->doExportSubscriptionsForChartmogul($monthToProcessBeginning, $monthToProcessEnd, $export_subscriptions_file_path);
						$s3->putObject(array(
								'Bucket' => $bucket,
								'Key' => $monthlyKey,
								'SourceFile' => $export_subscriptions_file_path
